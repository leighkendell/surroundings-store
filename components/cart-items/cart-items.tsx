...
const CartItems: React.FunctionComponent<Props> = React.memo(
  ({ products, client }) => {
    // Refs
    ...
    const hasNullVariants = products.some(
      product => product.node.variant === null
    );
    if (hasNullVariants) {
      ...
      <CartSection>
        ...
        <animated.div style={heightSpring}>
          <animated.ul
            ...
            style={updatingSpring}
            ref={listEl}
          >
            {sortedProducts.map(product => {
              const { id } = product.node;
              return (
                <CartItem key={id} data={product} setUpdating={setUpdating} />
              );
            })}
          </animated.ul>
        </animated.div>
        <Notification visible={updating}>
          {updating ? 'Updating cart...' : 'Cart updated!'}
        </Notification>
      </CartSection>
    );
  }
