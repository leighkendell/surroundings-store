...
const CartItem: React.FunctionComponent<Props> = React.memo(
  ({ data, setUpdating }) => {
    ...
    useEffect(() => {
      setUpdating(false);
    }, [data.node.quantity, setUpdating]);
    ...
    const handleRemove = (mutate: MutationFunction, checkout: Checkout) => {
      setUpdating(true);
                ...
                {(mutate, { loading }) => (
                  <>
                    <Input
                      ...
                      aria-label="Quantity"
                      onChange={event =>
                        handleQuantityChange(event, checkout, mutate)
                      }
                      disabled={loading}
                    />
                    <button
                      className={styles.button}
                      onClick={() => handleRemove(mutate, checkout)}
                      disabled={loading}
                    >
                      Remove
                    </button>
